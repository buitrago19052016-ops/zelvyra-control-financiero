...
function TransactionForm({ tipo, metas, defaultMetaId, initial, onCancel, onSave }) {
  ...
  return <Modal title={`${initial ? 'Editar' : 'Registrar'} ${palabra}`} onCancel={onCancel}>
    ...
    <label className="field-label">{isGasto ? '¿En qué te lo gastaste?' : 'Nota (opcional)'}</label>
    <input className="field-input" placeholder={isGasto ? 'Escríbelo tú: almuerzo, taxi, recibo de luz...' : 'Ej: sueldo, mercado...'} value={nota} onChange={(e) => setNota(e.target.value)} />
